...
import {FormControl, FormHelperText, InputLabel, MenuList} from '@material-ui/core';
...
class Header extends Component {
    constructor() {
        ...
        this.state = {
            ...
            invalidEmailInSignUpForm: "dispNone",
            signupFormPassword: "",
            signupFormPasswordRequired: "dispNone",
            signupFormContactNumber: "",
            signupFormContactNumberRequired: "dispNone",
            invalidLoginContact: "dispNone",
            invalidLoginPassword: "dispNone",
            NotRegisteredUser: "dispNone",
            invalidPasswordText: "dispNone",
            contactNumberRegistered: "dispNone",
            invalidContactText: "dispNone",
            snackBarOpen: false,
            snackBarMessage: "",
            transition: Fade,
    ...
    signUpFormPasswordChangeHandler = (event) => {
        this.setState({
            ...this.state,
            signupFormPassword: event.target.value,
        })
    }
    ...
    signUpFormContactNoChangeHandler = (event) => {
        this.setState({
            ...this.state,
            signupFormContactNumber: event.target.value,
        })
    }
            ...
            xhrLogin.addEventListener("readystatechange", function () {
                if (this.readyState === 4) {
                    if (xhrLogin.status === 200) {
                        ...
                        let loginResponse = JSON.parse(this.responseText);
                        let NotRegisteredUser = "dispNone"
                        let invalidLoginPassword = "dispNone"
                        if (loginResponse.code === 'ATH-001') {
                            NotRegisteredUser = "dispBlock"
                        }
                        if (loginResponse.code === 'ATH-002') {
                            invalidLoginPassword = "dispBlock"
                        }
                        that.setState({
                            ...that.state,
    ...
    signUpButtonClickHandler = () => {
        ...
        if (this.signUpFormValidation()) {
            let dataSignUp = JSON.stringify({
                "contact_number": this.state.signupFormContactNumber,
                ...
                if (this.readyState === 4) {
                    ...
                    if (xhrSignUp.status === 400) {
                        ...
                        if (responseData.code === 'SGR-001') {
                            that.setState({
                                ...that.state,
                                contactNumberRegistered: "dispBlock"
                            })
                        }
                    }
                }
    ...
    signUpFormValidation = () => {
        ...
        let signupFormPasswordRequired = "dispNone";
        let signUpContactNumberRequired = "dispNone";
        let invalidPasswordText = "dispNone";
        let invalidContactText = "dispNone";
        let invalidEmailInSignUpForm = "dispNone";
        ...
        if (this.state.email !== "") {
            ...
            if (!(/^\w+([-]?\w+)*@\w+([-]?\w+)*(\.\w+)+$/.test(this.state.email))) {
                invalidEmailInSignUpForm = "dispBlock"
                signUpFormValid = false;
            }
        }
        if (this.state.signupFormContactNumber === "") {
            signUpContactNumberRequired = "dispBlock";
            signUpFormValid = false;
        }
        if (this.state.signupFormContactNumber !== "") {
            const contactNo = "[1-9][0-9]{9}";
            if (!this.state.signupFormContactNumber.match(contactNo)) {
                invalidContactText = "dispBlock"
                signUpFormValid = false;
            }
        }
        if (this.state.signupFormPassword === "") {
            signupFormPasswordRequired = "dispBlock";
            signUpFormValid = false;
        }
        if (this.state.signupFormPassword !== "") {
            if (!this.isPasswordValid(this.state.signupFormPassword)) {
                invalidPasswordText = "dispBlock"
                signUpFormValid = false;
        ...
        this.setState({
            ...
            invalidContactText: invalidContactText,
            signupFormPasswordRequired: signupFormPasswordRequired,
            signupFormContactNumberRequired: signUpContactNumberRequired,
            invalidEmailInSignUpForm: invalidEmailInSignUpForm,
        ...
        return (
            <div>
                <header className="page-header">
                    ...
                    <span className="header-search-box">
                            <Input className={classes.searchText}
                                   ...
                                   onChange={this.inputSearchFieldChangeHandler}/>
                        </span>
                    }

                    {this.state.loggedIn !== true ?
                        <Button className="header-login-button" size="large" variant="contained"
                                onClick={this.loginButtonOnClickHandler}>
                            <AccountCircle className="header-login-button-icon"/>
                            LOGIN
                        </Button>
                        : <Button className="header-profile-button" style={{color: "#c2c2c2"}} size="large" variant="text"
                                  onClick={this.profileMenuButtonClickHandler}>
                            <AccountCircle className="header-profile-button-icon" htmlColor="#c2c2c2"/>
                            {this.state.loggedInUserName}
                        </Button>
                    }
                    <Menu id="profile-menu" anchorEl={this.state.anchorEl} open={this.state.isMenuOpen}
                          onClose={this.profileMenuButtonClickHandler}>
                        <MenuList className="menu-list">
                            <Link to={"/profile"} className={classes.menuItems} underline="none" color={"default"}>
                                <MenuItem className={classes.menuItems} onClick={this.onMyProfileClicked} disableGutters={false}>My
                                    profile</MenuItem>
                            </Link>
                ...
                <Modal
                    ...
                    style={customStyles}
                >
                    <Tabs className="header-login-modal-tabs" value={this.state.value}
                          onChange={this.loginSignupTabsChangeHandler}>
                    ...
                    <TabContainer>
                        <FormControl required className="model-form-control">
                            ...
                            <Input id="login-contact-no" className="input-fields" fullWidth={true} type="text"
                                   logincontactno={this.state.loginContactNumber} onChange={this.loginContactNumberChangeHandler}
                        ...
                        <FormControl required className="model-form-control">
                            ...
                            <Input id="login-password" className="input-fields" type="password"
                                   loginpassword={this.state.loginFormPassword} fullWidth={true}
                                   onChange={this.loginPasswordChangeHandler} value={this.state.loginFormPassword}/>
                            <FormHelperText className={this.state.loginFormPasswordRequired}>
                                <span className='red'>required</span>
                            </FormHelperText>
                            <FormHelperText className={this.state.invalidLoginPassword}>
                                <span className="red">Invalid Credentials</span>
                            </FormHelperText>
                            <FormHelperText className={this.state.NotRegisteredUser}>
                        ...
                        <br/>
                        <br/>
                        <Button variant="contained" className="header-form-button" color="primary"
                    ...
                    <TabContainer>
                        <FormControl required className="model-form-control">
                            ...
                            <Input id="first-name" className="input-fields" firstname={this.state.signupFormFirstName}
                                   fullWidth={true} onChange={this.firstNameChangeHandler} value={this.state.signupFormFirstName}/>
                        ...
                        <FormControl className="model-form-control">
                            ...
                            <Input id="last-name" className="input-fields" lastname={this.state.signupFormLastName} fullWidth={true}
                                   onChange={this.lastNameChangeHandler} value={this.state.signupFormLastName}/>
                        </FormControl>
                        <br/>
                        <br/>
                        <FormControl required className="model-form-control">
                            <InputLabel htmlFor="email">Email</InputLabel>
                            <Input id="email" className="input-fields" type="email" email={this.state.email} fullWidth={true}
                                   onChange={this.emailChangeHandler} value={this.state.email}/>
                            <FormHelperText className={this.state.signupFormEmailRequired}>
                                <span className="red">required</span>
                            </FormHelperText>
                            <FormHelperText className={this.state.invalidEmailInSignUpForm}>
                        ...
                        <FormControl required className="model-form-control">
                            ...
                            <Input id="sign-up-password" className="input-fields" type="password"
                                   signuppassword={this.state.signupFormPassword} fullWidth={true}
                                   onChange={this.signUpFormPasswordChangeHandler} value={this.state.signupFormPassword}/>
                            <FormHelperText className={this.state.signupFormPasswordRequired}>
                                <span className="red">required</span>
                            </FormHelperText>
                            <FormHelperText className={this.state.invalidPasswordText}>
                        ...
                        <FormControl required className="model-form-control">
                            ...
                            <Input id="sign-up-contactNo" className="input-fields"
                                   signupcontactno={this.state.signupFormContactNumber} fullWidth={true}
                                   onChange={this.signUpFormContactNoChangeHandler} value={this.state.signupFormContactNumber}/>
                            <FormHelperText className={this.state.signupFormContactNumberRequired}>
                                <span className="red">required</span>
                            </FormHelperText>
                            <FormHelperText className={this.state.invalidContactText}>
                                <span className="red">Contact No. must contain only numbers and must be 10 digits long</span>
                            </FormHelperText>
                            <FormHelperText className={this.state.contactNumberRegistered}>
                        ...
                        <br/>
                        <Button variant="contained" className="header-form-button" color="primary"
                                onClick={this.signUpButtonClickHandler}>SIGNUP</Button>
                    </TabContainer>
                    }
                </Modal>
                <div>
